    ...
    def run_batch(self, items: List[Dict]) -> List[Dict]:
        ...
        for agent_idx, agent in enumerate(agent_pbar):
            ...
            if not is_last_agent:
                # First agent can optionally generate text for structured reasoning
                ...
                # Handle model switching with clean text + latent separation
                ...
                # Last agent - need to handle model switching here too
                if model_switched and cumulative_latent_hiddens is not None:
                    ...
                    # Model switch but no latent hiddens - re-encode prompts with new model
                    prompt_encoded = agent_model.tokenizer(
                        cumulative_prompts,
                        return_tensors="pt",
                        padding=True,
